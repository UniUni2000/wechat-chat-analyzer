...
def find_decrypted_dir(search_path: str) -> str:
    ...
    """
    # 检查是否直接是解密目录
    if os.path.isdir(os.path.join(search_path, "message")) and \
       os.path.isdir(os.path.join(search_path, "contact")):
        return search_path
...
def load_contacts(db_dir: str) -> dict:
    ...
    try:
        ...
        if 'alias' in columns:
            ...
            rows = conn.execute("SELECT username, nick_name, remark, wechat_id FROM contact").fetchall()
...
def resolve_friend(contacts: dict, friend_input: str) -> str:
    ...
    if len(candidates) == 1:
        ...
        for wxid, reason in candidates[:10]:
            info = contacts[wxid]
            print(f"  - {wxid} ({reason}, 昵称: {info['nick_name']}, 备注: {info['remark']})")
...
def _find_message_table(db_dir: str, wxid: str) -> list:
    ...
    for filename in sorted(os.listdir(message_dir)):
        if filename.startswith("message_") and filename.endswith(".db") and \
           "fts" not in filename and "resource" not in filename:
...
def extract_messages(db_dir: str, wxid: str, my_name: str = "我",
                     friend_display: str = None) -> list:
    """提取指定好友的所有1对1文本聊天记录。
...
def load_chat_data(db_dir: str, friend: str, my_name: str = "我") -> dict:
    ...
    try:
        ...
        print(f"[+] 共提取 {len(all_messages)} 条消息 (其中文本 {len(text_messages)} 条)")
